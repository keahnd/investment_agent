...
def fetch_prices(tickers):
    ...
    Returns:
        DataFrame object with ticker information
    """
    all_tickers = list(tickers) + ['SPY', 'IWM', 'IVE', 'IVW', '^IRX']
    start = END_DATE - timedelta(days=LOOKBACK_DAYS)
    data = yf.download(all_tickers, start=start, end=END_DATE, auto_adjust=True, progress=False, threads=False)['Close'].dropna()
    dt = 1 / 252  # daily step
...
def compute_returns(raw, ticker):
...
def run_factor_models(returns, factors):
...
def fetch_valuation_metrics(ticker: str) -> dict:
    ...
    return {
        ...
        'fwd_pe':    info.get('forwardPE'),           # Forward P/E
        'ttm_pe':    info.get('trailingPE'),          # TTM P/E
        'ev_ebitda': info.get('enterpriseToEbitda'),  # EV/EBITDA
    }

def build_covariance(df_returns):
    print("Not implemented")

def run_optimisation(mu, Q_covar):
    print("Not implemented")
...
def main():
    curr_port = load_portfolio("portfolio.csv")
    tickers = curr_port["ticker"]
