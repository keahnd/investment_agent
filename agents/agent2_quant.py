...
def fetch_prices(tickers: list[str], start_date: date, today: date, force_refresh: bool = False) -> pd.DataFrame:
    ...
    if not force_refresh and os.path.exists(CACHE_FILE):
        ...
        if missing_tickers:
            ...
            new_cols = _download_with_retry(missing_tickers, start_date, today)
        ...
        data = _download_with_retry(all_tickers, start_date, today)
        data.to_parquet(CACHE_FILE)
    ...
    for ticker in tickers:
        ...
        if fm and g:
            mu_sigma[ticker] = {
                ...
                "s_current":    float(raw_prices[ticker].iloc[-1]),
                "is_fallback":  False
            }
        else:
            ...
            mu_sigma[ticker] = {
                "mu_annual":    0.07,
                "sigma_annual": 0.20,
                "s_current":    float(raw_prices[ticker].iloc[-1]) if ticker in raw_prices else None,
                "is_fallback":  True,
            }
